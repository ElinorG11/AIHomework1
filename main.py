...
def plot_distance_and_expanded_wrt_weight_figure(
    ...
    p1, = ax1.plot(weights, total_cost, 'b-',
                   label="Plot of the total distances")  # TODO: pass the relevant params instead of `...`.
    ...
    p2, = ax2.plot(weights, total_nr_expanded, 'r-',
                   label="Plot of the total expanded")  # TODO: pass the relevant params instead of `...`.
...
def run_astar_for_weights_in_range(heuristic_type: HeuristicFunctionType, problem: GraphProblem, n: int = 30,
    ...
    for weight in weights:
        aStar = AStar(heuristic_type, weight, max_nr_states_to_expand)
    ...
    plot_distance_and_expanded_wrt_weight_figure(problem.name, chosen_weights, costs, expanded_states)
...
def mda_problem_with_astar_epsilon_experiments():
    ...
    astarEpsilon = AStarEpsilon(heuristic_function_type=MDAMSTAirDistHeuristic, within_focal_priority_function=within_focal_h_sum_priority_function,
...
def mda_problem_anytime_astar_experiments():
    ...
    anytimeAstar = AnytimeAStar(heuristic_function_type=MDAMSTAirDistHeuristic,max_nr_states_to_expand_per_iteration=1000)
