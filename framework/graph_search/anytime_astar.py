    ...
    def solve_problem(self, problem: GraphProblem) -> SearchResult:
        with Timer(print_title=False) as timer:
            ...
            while (high_heuristic_weight - low_heuristic_weight) > 0.01:
                ...
                mid_heuristic_weight = (high_heuristic_weight + low_heuristic_weight) / 2
                astar = AStar(heuristic_function_type=self.heuristic_function_type,heuristic_weight=mid_heuristic_weight,max_nr_states_to_expand=self.max_nr_states_to_expand_per_iteration)
                res = astar.solve_problem(problem=problem)
